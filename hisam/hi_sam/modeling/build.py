    ...
    if checkpoint is not None:
        ...
        if args.hier_det:
            ...
            if not contain_hi_decoder:
                ...
                with open(os.path.join(pretrained_dir, args.model_type+'_maskdecoder.pth'), "rb") as f:
...
def _build_efficient_sam(encoder_patch_embed_dim, encoder_num_heads, args):
    ...
    if checkpoint is not None:
        ...
        if args.hier_det:
            ...
            if not contain_hi_decoder:
                pretrained_dir = os.path.join(os.path.dirname(__file__), '..', '..', 'pretrained_checkpoint')
                with open(os.path.join(pretrained_dir, args.model_type+'_maskdecoder.pth'), "rb") as f_maskdecoder:
